        ...
        try {
            ...
            try {
                ...
                if (Array.isArray(servicesRes)) {
                    ...
                    servicesData = Array.isArray(servicesRes.data) ? servicesRes.data : (servicesRes.data.services || []);
                }
                ...
                {selectedAppointment && (
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-sm text-gray-500 block mb-1">Bệnh nhân</label>
                            ...
                            <div>
                                <label className="text-sm text-gray-500 block mb-1">Ngày giờ</label>
                                <div className="font-medium">
                                    {format(new Date(selectedAppointment.appointmentDate), 'dd/MM/yyyy HH:mm', { locale: vi })}
                                </div>
                            </div>
                            <div>
                                <label className="text-sm text-gray-500 block mb-1">Trạng thái</label>
                                <div>
                                    <span
                                        className={styles.statusBadge}
                                        style={{
                                    ...
                                    >
                                        {APPOINTMENT_STATUS_LABELS[selectedAppointment.status as keyof typeof APPOINTMENT_STATUS_LABELS]}
                                    </span>
                                </div>
                            </div>
                            {selectedAppointment.confirmedBy && (
                                <div>
                                    <label className="text-sm text-gray-500 block mb-1">Người xác nhận</label>
                                    <div className="font-medium">
                                        {typeof selectedAppointment.confirmedBy === 'object'
                        ...
                        {typeof selectedAppointment.patientId === 'object' && selectedAppointment.patientId && (
                            <div className="mt-4 pt-4 border-t border-gray-100">
                                <h4 className="font-medium mb-2 text-gray-900">Thông tin bệnh nhân</h4>
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <span className="text-gray-500">Giới tính:</span>{' '}
                                        {selectedAppointment.patientId.gender === 'male' ? 'Nam' :
                                            selectedAppointment.patientId.gender === 'female' ? 'Nữ' : 'Khác'}
                                    </div>
                                    <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        ...
                        {!selectedAppointment.confirmedBy && (
                            <div className="p-3 bg-yellow-50 text-yellow-800 text-sm rounded border border-yellow-100">
                                <strong>Lưu ý:</strong> Lịch hẹn này chưa có thông tin nhân viên xác nhận.
                                Bạn vẫn có thể bắt đầu khám, nhưng vui lòng kiểm tra kỹ thông tin.
                            </div>
                        )}
                        <div style={{ padding: '12px', backgroundColor: '#f3f4f6', borderRadius: '8px', fontSize: '14px', color: '#6b7280' }}>
                            <strong>Lưu ý:</strong> Nhân viên sẽ được tự động chọn dựa trên người xác nhận lịch hẹn (nếu có).
                        </div>
                    </div>
                )}
            </Modal>
