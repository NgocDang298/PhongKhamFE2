...
export default function DoctorExaminationsPage() {
    ...
    const loadData = async () => {
        try {
            ...
            } else if (examsRes?.examinations && Array.isArray(examsRes.examinations)) {
                examinationsData = examsRes.examinations;
            } else if (examsRes?.data) {
                if (Array.isArray(examsRes.data)) {
                    examinationsData = examsRes.data;
                } else if (examsRes.data?.examinations && Array.isArray(examsRes.data.examinations)) {
            ...
            if (isAuthenticated) {
                ...
                try {
                    ...
                    } else if ((servicesRes as any)?.services && Array.isArray((servicesRes as any).services)) {
                        servicesData = (servicesRes as any).services;
                    } else if (servicesRes?.data) {
                        servicesData = Array.isArray(servicesRes.data) ? servicesRes.data : ((servicesRes.data as any).services || []);
                    }
                ...
                try {
                    ...
                    } else if ((testServicesRes as any)?.services && Array.isArray((testServicesRes as any).services)) {
                        testServicesData = (testServicesRes as any).services;
                    } else if (testServicesRes?.data) {
                        testServicesData = Array.isArray(testServicesRes.data) ? testServicesRes.data : ((testServicesRes.data as any).services || []);
                    }
    ...
    const handleStartExamination = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const selectedAppointment = appointments.find(apt => apt._id === startFormData.appointmentId);
    ...
    const handleUpdateExamination = async (e: React.FormEvent) => {
        ...
        try {
            await examinationService.updateExamination(selectedExamination._id, updateFormData);
            setIsUpdateModalOpen(false);
            setSelectedExamination(null);
            setUpdateFormData({ diagnosis: '', treatment: '', doctorNote: '', resultSummary: '' });
            loadData();
        } catch (error: any) {
            alert(error.message || 'Có lỗi xảy ra');
        }
    ...
    const handleCompleteExamination = async () => {
        if (!selectedExamination) return;
        try {
            await examinationService.completeExamination(selectedExamination._id, updateFormData);
            setIsUpdateModalOpen(false);
            setSelectedExamination(null);
            setUpdateFormData({ diagnosis: '', treatment: '', doctorNote: '', resultSummary: '' });
            loadData();
    ...
    const handleOpenUpdateModal = async (exam: any) => {
        ...
        try {
            const testRequestsRes = await testRequestService.getTestRequestsByExamination(exam._id);
    ...
    const handleCreateTestRequest = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const testRequestsRes = await testRequestService.getTestRequestsByExamination(selectedExamination._id);
            let testRequestsData: any[] = [];
            if (Array.isArray(testRequestsRes)) {
                testRequestsData = testRequestsRes;
            } else if (testRequestsRes?.data && Array.isArray(testRequestsRes.data)) {
    ...
    return (
        <DashboardLayout navItems={navItems} title="Ca khám">
            ...
            <Card>
                ...
                <CardBody>
                    {examinations.length === 0 ? (
                        <div className={styles.empty}>Chưa có ca khám nào</div>
                    ) : (
                        <Table>
                            <TableHeader>
                                <TableRow>
                                ...
                                {examinations.map((exam) => (
                                    <TableRow key={exam._id}>
                                        ...
                                        <TableCell>
                                            ...
                                            >
                                                {EXAMINATION_STATUS_LABELS[exam.status as keyof typeof EXAMINATION_STATUS_LABELS]}
                                            </span>
                                        </TableCell>
                                        <TableCell>
                                            <Button
            ...
            >
                <form onSubmit={handleStartExamination} className={styles.form}>
                    <Select
                        label="Chọn lịch hẹn"
                        name="appointmentId"
                        options={[
                            ...
                            ...appointments
                                ...
                                .map((apt) => ({
                                    value: apt._id,
                                    label: `${typeof apt.patientId === 'object' && apt.patientId ? apt.patientId.fullName : 'N/A'} - ${format(new Date(apt.appointmentDate), 'dd/MM/yyyy HH:mm', { locale: vi })}`,
                                })),
                        ]}
                        value={startFormData.appointmentId}
                        onChange={(e) => setStartFormData({ ...startFormData, appointmentId: e.target.value })}
                        required
                        fullWidth
                    />
                    <Select
                        ...
                        options={[
                            ...
                            ...services.map((s) => ({ value: s._id, label: `${s.name} - ${s.price.toLocaleString('vi-VN')}đ` })),
                        ]}
                        value={startFormData.serviceId}
                        onChange={(e) => setStartFormData({ ...startFormData, serviceId: e.target.value })}
                        required
                        fullWidth
                    />
                    <div style={{ padding: '12px', backgroundColor: '#f3f4f6', borderRadius: '8px', fontSize: '14px', color: '#6b7280' }}>
                        <strong>Lưu ý:</strong> Nhân viên sẽ được tự động chọn dựa trên người xác nhận lịch hẹn.
                    </div>
                </form>
            </Modal>
            ...
            >
                <form onSubmit={handleUpdateExamination} className={styles.form}>
                    <Textarea
                        label="Chẩn đoán"
                        name="diagnosis"
                        value={updateFormData.diagnosis}
                        onChange={(e) => setUpdateFormData({ ...updateFormData, diagnosis: e.target.value })}
                        fullWidth
                        rows={3}
                    />
                    <Textarea
                        ...
                        onChange={(e) => setUpdateFormData({ ...updateFormData, treatment: e.target.value })}
                        fullWidth
                        rows={3}
                    />
                    <Textarea
                        ...
                        onChange={(e) => setUpdateFormData({ ...updateFormData, doctorNote: e.target.value })}
                        fullWidth
                        rows={2}
                    />
                    <Textarea
                        ...
                        onChange={(e) => setUpdateFormData({ ...updateFormData, resultSummary: e.target.value })}
                        fullWidth
                        rows={2}
                    />
                    ...
                    <div className="mt-6 border-t pt-4">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            {selectedExamination?.status === 'processing' && (
                                <Button type="button" size="sm" onClick={handleOpenTestRequestModal}>
                                    + Thêm yêu cầu
                                </Button>
                            )}
                        </div>
                        ...
                        {testRequests.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {testRequests.map((req, index) => {
                                    ...
                                    return (
                                        <div key={req._id || index} className="p-3 border rounded-lg bg-gray-50 flex justify-between items-center">
                                            <div>
                                                <div className="font-medium">
                                                    {typeof req.serviceId === 'object' ? req.serviceId.name : 'Dịch vụ xét nghiệm'}
                                                </div>
                                                <div className="text-sm text-gray-500 mt-1">
                                                    {req.testType}
                                                </div>
                                                <div className="text-xs text-gray-400 mt-1">
                                                    Y tá: {typeof req.labNurseId === 'object' ? req.labNurseId.fullName : 'Chưa phân công'}
                                                </div>
                                            </div>
                                            <span
                                            ...
                                            >
                                                {req.status === 'waiting' ? 'Chờ xử lý' :
                                                    req.status === 'processing' ? 'Đang thực hiện' : 'Hoàn thành'}
                                            </span>
                                        </div>
                                    );
                                })}
            ...
            >
                <form onSubmit={handleCreateTestRequest} className={styles.form}>
                    <Select
                        label="Chọn dịch vụ xét nghiệm"
                        value={testRequestFormData.serviceId}
                        onChange={(e) => setTestRequestFormData({ ...testRequestFormData, serviceId: e.target.value })}
                        options={[
                            { value: '', label: 'Chọn dịch vụ' },
                            ...testServices.map(s => ({
                                value: s._id,
                    ...
                    <Select
                        ...
                        onChange={(e) => setTestRequestFormData({ ...testRequestFormData, labNurseId: e.target.value })}
                        options={[
                    ...
                    <Input
                        ...
                        onChange={(e) => setTestRequestFormData({ ...testRequestFormData, testType: e.target.value })}
